...
RSpec.describe DealershipsController, type: :controller do
  ...
  describe "GET #index" do
    ...
    it 'renders the index template' do
      ...
      it 'has dealerships in the dealerships instance variable' do
        ...
        get :index
        expect(assigns(:dealerships).length).to eq(2)
        expect(assigns(:dealerships).last.name).to eq('Dealer_1')
      end
    end
  ...
  describe "GET #new" do
    ...
    before(:all) do
      @dealership_params = { dealership: { name: 'Test Dealer', inventory: '100' } }
    end
    ...
    describe 'failure' do
      ...
      dealership2 = { dealership: { name: 'Things'}}
  ...
  describe "PUT #update" do
    it 'sets the car instance variable' do
      put :update, { id: dealership.id, dealership: { name: 'Stuff', inventory: 1000 }}
      expect(assigns(:dealership).id).to eq(dealership.id)
    end

    it "updates the dealership" do
      put :update, { id: dealership.id, dealership: { name: 'Stuff'}}
      expect(dealership.reload.name).to eq('Stuff')
    end
    ...
    it "sets a flash message on success" do
      ...
      describe 'update failures' do
        before (:each) do
          Dealership.create(name: "Yep", inventory: 1000)
        end

        it "renders edit on fail" do
          put :update, { id: dealership.id, dealership: { name: 'Yes', inventory: nil } }
          expect(response).to render_template(:edit)
        end
  ...
  describe "DELETE #destroy" do
    ...
    it "destroys the dealership" do
      dealership # this is let dealership at top
      expect(Dealership.count).to eq(1)
      delete :destroy, id: dealership.id
      expect(Dealership.count).to eq(0)
    end
